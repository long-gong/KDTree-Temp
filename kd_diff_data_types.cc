...
void kdtree_demo(const size_t nSamples,
  ...
  std::vector<std::vector<T>> samples(nSamples, std::vector<T>(dim, 0));
  const T max_range = 128;
  ...
  // Generate points:
  for (unsigned i = 0; i < nSamples; ++i) generateRandomPoint(&samples[i][0], dim, rng);
...

  {
    ...
    // ------------------------------------------------------------
    typedef KDTreeVectorOfVectorsAdaptor<std::vector<std::vector<T>>, T> my_kd_tree_t;
...
template<typename T>
void kdtree_demo_flat(const size_t nSamples,
                      const size_t dim,
                      unsigned seed = (unsigned) std::chrono::system_clock::now().time_since_epoch().count()) {
...

  {
    ...
    typedef KDTreeFlatVectorAdaptor<std::vector<int16_t>, int16_t, -1, long long> my_kd_tree_t;
    ...
    const size_t num_results = 1;
    std::vector<size_t> ret_indexes(num_results);
    std::vector<long long> out_dists_sqr(num_results);

    std::vector<std::pair<size_t, long long>> resutls(nq, {(size_t) 0, (long long) (0)});
    std::cout << "Start Query ...\n";
    boost::timer timer;
    timer.restart();
    {
...
int main() {
  ...
  kdtree_demo<int>((1u << 22u) /* samples */, SAMPLES_DIM /* dim */, 20190429u);
//  kdtree_demo_flat<int16_t>((1u << 22u) /* samples */, SAMPLES_DIM /* dim */, 20190429u);

//    kdtree_demo<float, KDTreeVectorOfVectorsAdaptor<std::vect>((1u << 22u) /* samples */, SAMPLES_DIM /* dim */, 20190429u);
  return 0;
}
